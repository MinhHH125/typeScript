...
function handleCreate() {
    ...
    if (!productName)
        return;
    const storeProduct = stores.find((product) => product.name.toLowerCase() === productName.toLowerCase());
    if (storeProduct && storeProduct.count > 0) {
        storeProduct.count -= 1;
        const cartProduct = carts.find((cart) => cart.name.toLowerCase() === productName.toLowerCase());
        if (cartProduct) {
...
function handleDelete() {
    ...
    if (!productName)
        return;
    const cartIndex = carts.findIndex((cart) => cart.name.toLowerCase() === productName.toLowerCase());
    if (cartIndex > -1) {
...
function main() {
    while (true) {
        const choice = prompt("Nhập C (Thêm), R (Xem), U (Cập nhật), D (Xóa), E (Thoát):");
        if (!choice)
            continue;
